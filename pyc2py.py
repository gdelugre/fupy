...
"""
  ...
  pyc2py is free software: you can redistribute it and/or modify
  it under the terms of the GNU Lesser General Public License as published by
  the Free Software Foundation, either version 3 of the License, or
  (at your option) any later version.
 
  pyc2py is distributed in the hope that it will be useful,
  but WITHOUT ANY WARRANTY; without even the implied warranty of
  MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
  GNU Lesser General Public License for more details.
 
  You should have received a copy of the GNU Lesser General Public License
  along with pyc2py.  If not, see <http://www.gnu.org/licenses/>.
...
class ImportFrom(Statement):
  ...
  @Statement.auto_indent
  def write(self, indent = ''):
    out = "from " + self.module.write() + " import " + self.name.write()
    if self.as_name:
      out += " as " + self.as_name.write()
    return out
...
class PythonCompiledModuleAttr:
  """
  ...
  def __decompile_block(self, insns, stack):
    ...
    while n < len(insns):
      ...
      if opname in self.conditional_jump_insns:
        ...
        if len(stack) == 0 or not isinstance(stack[-1], PythonCompiledModule):  
          raise PythonDecompilerError("Expected module on the stack.", addr, opname, arg)
        stack.append(PythonCompiledModuleAttr(stack[-1].module, Variable(arg)))
      elif opname == 'IMPORT_NAME':
        fromlist = stack.pop()
        level = stack.pop()
        stack.append(PythonCompiledModule(Variable(arg)))
      elif opname == 'IMPORT_STAR':
        if len(stack) == 0 or not isinstance(stack[-1], PythonCompiledModule):  
          raise PythonDecompilerError("Expected module on the stack.", addr, opname, arg)
        statements.append(ImportFrom(stack.pop().module, Variable('*')))
